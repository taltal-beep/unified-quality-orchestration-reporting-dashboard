        ...
        try:
            ...
            while True:
                ...
                if isinstance(payload, LogEvent):
                    ...
                    if (
                        int(payload.returncode) != 0
                        and run_id is not None
                        and self._failure_analysis_service is not None
                    ):
                        try:
                            self._failure_analysis_service.generate_summary(
                                run_id=str(run_id), force_refresh=False
                            )
                        except Exception:  # pragma: no cover - best-effort
                            logger_failure = getattr(self, "_logger", None)
                            if logger_failure is not None:
                                logger_failure.exception("Failed to generate AI failure summary")
