...
from uqo_core.run_history import RunSyncStatus, create_run, record_completed_run, update_run_status
...
def _split_args_for_spec(spec: EngineRunSpec) -> tuple[Sequence[str], Sequence[str], Sequence[str], Sequence[str]]:
    ...
    if spec.test_type == TestType.BEHAVE_NATIVE:
        return (), (), args, ()
    if spec.test_type == TestType.LOCUST:
        return (), (), (), args
...
def _build_run_config(spec: EngineRunSpec, *, db_run_id: str | None) -> RunConfig:
    pytest_args, behavex_args, behave_native_args, locust_args = _split_args_for_spec(spec)
    ...
    return RunConfig(
        ...
        locust_users=int(spec.locust_users),
        locust_spawn_rate=int(spec.locust_spawn_rate),
        locust_run_time=str(spec.locust_run_time),
        locust_only_summary=bool(spec.locust_only_summary),
