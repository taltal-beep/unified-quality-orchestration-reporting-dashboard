...
from uqo_core.run_history import CompletedRunView, RunSessionView, get_run, list_run_sessions
from uqo_core.services.delta_models import DeltaClassification, DeltaComparisonResult, MetricDelta
...
class DashboardService:
    ...
    def _build_report_links(self, *, latest_session: RunSessionView | None) -> DashboardReportLinks:
        ...
        locust = self._pick_link(links, "locust")
        behave = self._pick_link(links, "behave_native", "behavex", "behave")
        return DashboardReportLinks(allure=allure, locust=locust, behave=behave)
