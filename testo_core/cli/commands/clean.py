...
def clean(
    ...
    if not yes:
        print_fail(console, "Refusing to delete without --yes (or -y).")
    ...
    try:
        ...
        print_warn(console, f"Could not load config ({exc}); using ./artifacts for cleanup.")
    ...
    if removed:
        for p in removed:
            print_ok(console, f"Removed directory {p}")
    else:
    ...
    if docker:
        ...
        if code == 0:
            print_ok(console, "Docker prune completed for labeled containers.")
            if out.strip():
                console.print(f"[dim]{out.strip()}[/]")
        elif code == 127:
            print_warn(console, "docker CLI not found; skipping container prune.")
        else:
            print_fail(console, f"docker prune exited {code}: {out.strip()}")
            raise typer.Exit(code=int(EngineExitCode.INFRA_FAILURE))

    print_ok(console, "Clean finished.")
