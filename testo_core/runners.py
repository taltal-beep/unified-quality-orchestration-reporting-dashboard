...
def _host_repo_root() -> Path:
    # Repo root is the parent of `testo_core/`.
...
def run_audit_streaming(
    *,
    ...
    """
    Run Pytest → BehaveX → Native Behave (optional) into isolated
    ``allure-results/{pytest,behavex,behave_native}/`` directories (no overwrite).
    ...
    if enable_native_behave:
        ...
        phases.append((TestType.BEHAVE_NATIVE, "behave_native", "Behave (native)"))
