...
from testo_core.sandbox_api import (
    MOCK_BASE_URL,
...
def init_state() -> None:
    ...
    st.session_state.setdefault("config_names_by_id", {})
...
def _start_worker_multi(specs: list[EngineRunSpec], *, ci_mode: bool = False) -> None:
    ...
    _uqo_debug_log(
        hypothesis_id="H4",
        location="app.py:_start_worker_multi",
        message="enter _start_worker_multi",
        data={"specs_len": int(len(specs)), "ci_mode": bool(ci_mode)},
    )
    # endregion agent log
    ...
    def worker() -> None:
        ...
        _uqo_debug_log(
            hypothesis_id="H4",
            location="app.py:_start_worker_multi.worker",
            message="worker thread entered",
            data={},
        )
        # endregion agent log
        try:
    ...
    _uqo_debug_log(
        hypothesis_id="H4",
        location="app.py:_start_worker_multi",
        message="started worker thread",
        data={
            "specs_len": int(len(specs)),
            "worker_alive": bool(getattr(t, "is_alive", lambda: False)()),
            "events_q_set": bool(st.session_state.get("events_q") is not None),
        },
    )
    # endregion agent log
...
def _apply_run_result_to_session(item: RunResult) -> None:
    ...
    try:
        payload = {
            ...
            "runId": "pre-fix",
            "hypothesisId": "H3",
            "location": "app.py:_apply_run_result_to_session",
            "message": "apply RunResult",
            "data": {
    ...
    try:
        payload = {
            ...
            "hypothesisId": "H3",
            "location": "app.py:_apply_run_result_to_session",
            "message": "applied RunResult; updated state",
            "data": {
    ...
    if done_sentinel:
        try:
            payload = {
                ...
                "runId": "pre-fix",
                "hypothesisId": "H3",
                "location": "app.py:_apply_run_log_line_to_session",
                "message": "saw done marker",
                "data": {
                    "stream": str(stream),
                    "in_multi_run": bool(in_multi_run),
                    "multi_runs_remaining": int(st.session_state.get("multi_runs_remaining") or 0),
                    "batch_still_running": bool(batch_still_running),
                    "line_sample": str(line[:200]),
                },
                "timestamp": int(time.time() * 1000),
            }
            with open(
...
def _drain_events() -> None:
    ...
    if not q:
        ...
        if bool(st.session_state.get("running")):
            _uqo_debug_log(
                hypothesis_id="H4",
                location="app.py:_drain_events",
                message="running=true but events_q missing",
                data={
                    "worker_set": bool(st.session_state.get("worker") is not None),
                    "multi_run_active": bool(st.session_state.get("multi_run_active")),
                    "multi_runs_remaining": int(st.session_state.get("multi_runs_remaining") or 0),
                },
            )
    ...
    try:
        ...
        _uqo_debug_log(
            hypothesis_id="H4",
            location="app.py:_drain_events",
            message="draining events",
            data={
                "qsize": int(getattr(q, "qsize", lambda: -1)()),
                "worker_alive": bool(getattr(w, "is_alive", lambda: False)()) if w else False,
            },
        )
    except Exception:
        pass
    ...
    # Safety net: if the UI thinks we're running but the worker is dead and no events are queued,
    # stop the infinite rerun loop and unblock the UI.
    try:
        w = st.session_state.get("worker")
        worker_alive = bool(getattr(w, "is_alive", lambda: False)()) if w else False
        ...
        if bool(st.session_state.get("running")) and (not worker_alive) and qsize <= 0 and started_at > 0:
            if (time.time() - started_at) >= grace_s:
                ...
                _append_line("[ui] run aborted: worker not alive and queue empty")
                # region agent log
                _uqo_debug_log(
                    hypothesis_id="H4",
                    location="app.py:_drain_events",
...
def _render_hero_background(*, image_path: Path, height_px: int = 220) -> None:
...
st.markdown(
    """
    <style>
      /* App-only layout: hide Streamlit chrome */
      header[data-testid="stHeader"] { display: none; }
      #MainMenu { visibility: hidden; }
      footer { visibility: hidden; }

      /* Remove the blank space left when the header is hidden */
...
_render_hero_background(image_path=_logo_path, height_px=300)
...
with tab_exec:
    ...
    with left_cfg:
        ...
        for idx, cfg_state in enumerate(configs):
            ...
            with header_a:
                names_by_id: dict[str, str] = st.session_state.get("config_names_by_id") or {}
                default_label = f"Configuration {idx + 1}"
                existing_name = str(names_by_id.get(cfg_id) or "")
                name_key = f"name_{cfg_id}_{ui_seed}"
                new_name = st.text_input(
                    "Configuration name",
                    value=existing_name,
                    placeholder=default_label,
                    disabled=bool(st.session_state.running),
                    help="Rename this configuration (session-only). Leave empty to use the default label.",
                    label_visibility="collapsed",
                    key=name_key,
                )
                new_name = str(new_name or "").strip()
                if new_name:
                    names_by_id[cfg_id] = new_name
                else:
                    names_by_id.pop(cfg_id, None)
        ...
        if remove_id is not None:
            try:
                names_by_id: dict[str, str] = st.session_state.get("config_names_by_id") or {}
                names_by_id.pop(str(remove_id), None)
                st.session_state["config_names_by_id"] = names_by_id
            except Exception:
                pass
        ...
        if run_clicked:
            ...
            _uqo_debug_log(
                hypothesis_id="H4",
                location="app.py:run_clicked",
                message="Run clicked",
                data={
                    "running_before": bool(st.session_state.get("running")),
                    "configs": int(len(updated)),
                },
            )
            # endregion agent log
            bad: list[str] = []
            ...
            if bad:
                ...
                _uqo_debug_log(
                    hypothesis_id="H4",
                    location="app.py:run_clicked",
                    message="about to start worker",
                    data={"run_specs_len": int(len(run_specs))},
                )
                # endregion agent log
