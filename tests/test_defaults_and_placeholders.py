...
from engine.runners import LogEvent, RunResult, run_behave_native_streaming
...
def test_run_behave_native_streaming_placeholder(tmp_path: Path) -> None:
    gen = run_behave_native_streaming(target_repo=tmp_path, artifacts_root=tmp_path)
    ev = next(gen)
