...
import uqo_core.integrations as integrations
...
def test_push_to_prometheus_http_error_branch() -> None:
    ...
    with patch("uqo_core.integrations.prometheus_settings_from_env", return_value={"pushgateway_url": "http://x", "job_name": "uqo"}):
...
def test_push_to_prometheus_missing_url_branch() -> None:
    ...
    with patch("uqo_core.integrations.prometheus_settings_from_env", return_value={"pushgateway_url": None, "job_name": "uqo"}):
        ok, msg = integrations.push_to_prometheus(m)
    assert ok is False
