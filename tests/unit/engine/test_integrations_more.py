"""More coverage for ``engine.integrations`` (all mocked network / extractors)."""
...
def test_integration_status_from_env_false(monkeypatch: pytest.MonkeyPatch) -> None:
    from engine.integrations import integration_status_from_env
...
def test_push_to_prometheus_missing_url(sample_metrics: RunMetrics) -> None:
    from engine.integrations import push_to_prometheus
...
def test_auto_push_metrics_no_extract(tmp_path: Path) -> None:
    from engine.integrations import auto_push_metrics_if_enabled

    with patch("engine.integrations.extract_best", return_value=None):
...
def test_auto_push_metrics_happy_path(tmp_path: Path, sample_metrics: RunMetrics) -> None:
    ...
    fake_em = MagicMock()
    with patch("engine.integrations.extract_best", return_value=fake_em):
        with patch("engine.integrations.to_run_metrics", return_value=sample_metrics):
            with patch("engine.integrations.push_to_influxdb", return_value=(True, "ok")):
                with patch("engine.integrations.push_to_prometheus", return_value=(True, "ok")):
                    out = auto_push_metrics_if_enabled(
                        artifacts_root=tmp_path,
                        run_id="rid",
                        auto_influx=True,
