...
def test_run_streaming_does_not_override_shared_allure_results_dir(tmp_path: Path) -> None:
    ...
    with patch("testo_core.runners.build_command", side_effect=fake_build_command):
        with patch("testo_core.runners._run_in_ephemeral_container_streaming") as run:
...
def test_audit_does_not_generate_unified_allure_html(tmp_path: Path) -> None:
    ...
    with patch("testo_core.runners.run_streaming", side_effect=fake_run_streaming):
    ...
    assert all(p.name in {"pytest", "behavex", "behave_native"} for p in calls)
